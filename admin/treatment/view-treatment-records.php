...
$select = "SELECT name, description, tmr.date 
    ...
    WHERE category=1 && $session_id=patient_id && treatment_record_id=tmr.id && tm.id=tmr.treat_med_id";
$result = mysqli_query($conn, $select);
// echo $select;
$treatment_list_record = [];
...
if(mysqli_num_rows($result))  {
  foreach($result as $row)  {
    ...
    $date = $row['date'];  
    array_push($treatment_list_record, array('name' => $name, 'description' => $description, 'date' => $date));
  } 
...
<div class="d-flex" id="wrapper"> 
  ...
  <div id="page-content-wrapper" >
    ...
    <div class="container-fluid">
      <div class="row bg-light m-3"><h3>View Treatment Records</h3>
      <div class="container default table-responsive">
          ...
          <table class="table mt-5 table-striped table-responsive table-lg table-bordered table-hover display" id="datatables" >
            ...
            <tbody>
              <?php 
                foreach ($treatment_list_record as $key => $value) {
              ?>    
                <tr>
